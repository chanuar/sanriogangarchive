...
try {
  preference = localStorage.getItem("sg-effects");
} catch {
  /* Storage is optional. */
}
...
document
  ...
  .forEach((button) => {
    button.addEventListener("click", () =>
      strip.scrollBy({
        left: Number(button.dataset.galleryStep) * strip.clientWidth * 0.8,
        behavior:
...
const reveal = new IntersectionObserver(
  (entries) => {
    entries.forEach((entry) => {
      ...
      if (document.documentElement.dataset.effects === "on")
